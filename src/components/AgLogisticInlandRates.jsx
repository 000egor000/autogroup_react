...
import { getRequest, putRequest, postRequest } from '../base/api-request'
...
const AgLogisticInlandRates = ({}) => {
  ...
  useEffect(() => getInlandRatesArray(), [page, limit, titleRates])
  ...
  const getInlandRatesArray = () => {
    ...
    url = `/api/v1/rates-fees/inland-rates?carrier_id=4&page=${page}&limit=${limit}`
    getRequest(url, {
      Authorization: `Bearer ${window.sessionStorage.getItem('access_token')}`,
    })
      .then((res) => {
        res['inlandRates'].map((elem) => container.push(Object.entries(elem)))
  ...
  const getSearchInlandRates = () => {
    ...
    postRequest(`/api/v1/rates-fees/inland-rates/search?page=${page}`, {
      ...
      .catch((err) => {
        showInlandRatesArray([])
        dispatch(showLoder({ locationsSearch: 0, status: err.status }))
      })
  }

  const changeTableOrEdit = () => {
    dispatch(showLoder({ changeTableOrEdit: 1 }))
    putRequest(`/api/v1/rates-fees/inland-rates/${stateCell}`, {
...
AgLogisticInlandRates.propTypes = {
  currentRates: PropTypes.string,
  viewBlock: PropTypes.func,
}
export default AgLogisticInlandRates

...
const AgLogisticDestinationInlandRates = ({}) => {
  ...
  const getInlandRatesArray = () => {
    ...
    url = `/api/v1/rates-fees/inland-rates-ag-carter?carter_id=15&page=${page}&limit=${limit}`
    getRequest(url, {
      Authorization: `Bearer ${window.sessionStorage.getItem('access_token')}`,
    })
      .then((res) => {
        res['inlandRatesAgCarters'].map((elem) =>
          container.push(Object.entries(elem))
        )
  ...
  const changeTableOrEdit = () => {
    ...
    putRequest(`/api/v1/rates-fees/inland-rates-ag-carter/${stateCell}`, {
...
AgLogisticDestinationInlandRates.propTypes = {
  currentRates: PropTypes.string,
  viewBlock: PropTypes.func,
}
export default AgLogisticDestinationInlandRates

...
const Masters = (props) => {
  ...
  const getArray = () => {
    ...
    getRequest(`/api/v1/masters?page=${page}`, {
      Authorization: `Bearer ${window.sessionStorage.getItem('access_token')}`,
    })
      .then((res) => {
        if (title === 'Мастера') {
          setDataMaster(res.masters.filter((el) => el.email !== email))
          setPaginationValue(res.pagination - 1)
        } else {
          setDataMaster(res.masters)
      ...
      <div className="itemContainer-inner">
        ...
        >
          <div className="btnTransport">
            {/* <h1 className='titleInfo'>Пользователи: мастер</h1> */}
            {viewBlock(1) && (
              <button className="btnInfo">
                <Link to={'/masters/create'}>Добавить мастера</Link>
              </button>
            )}
          </div>
        </div>
                  ...
                  <Cell>
                    {(rowData, rowIndex) => {
                      return (
                        <div>
                          <Link
                            to={viewBlock(2) && `/masters/${rowData.id}/edit`}
                          >
                            <span>
                          ...
                          <div className="Dropdown">
                            <div className="DropdownShow">
                              ...
                              {viewBlock(2) && (
                                ...
                                >
                                  <button>
                                    <Link to={`/masters/${rowData.id}/edit`}>
                                      <Edit />
                                    </Link>
                                  </button>
                                </Whisper>

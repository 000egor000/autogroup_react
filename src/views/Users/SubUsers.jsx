...
const SubUser = (props) => {
  ...
  const getArray = () => {
    ...
    getRequest(`/api/v1/sub-users?page=${page}`, {
      Authorization: `Bearer ${window.sessionStorage.getItem('access_token')}`,
    })
      .then((res) => {
        if (title === 'Субпользователи') {
          setDataMaster(res.sub_users.filter((el) => el.email !== email))
          setPaginationValue(res.pagination - 1)
        } else {
          setDataMaster(res.sub_users)
          ...
          <div className="btnTransport">
            ...
            {viewBlock(9) && (
              <button className="btnInfo">
                <Link to={'/subusers/create'}>Добавить субпользователя</Link>
              </button>
            )}
          </div>
        </div>
                  ...
                  <Cell>
                    {(rowData, rowIndex) => {
                      return (
                        <div>
                          <Link
                            to={viewBlock(10) && `/subusers/${rowData.id}/edit`}
                          >
                            <span>
                          ...
                          <div className="Dropdown">
                            <div className="DropdownShow">
                              ...
                              {viewBlock(10) && (
                                ...
                                >
                                  <button>
                                    <Link to={`/subusers/${rowData.id}/edit`}>
                                      <Edit />
                                    </Link>
                                  </button>
                                </Whisper>

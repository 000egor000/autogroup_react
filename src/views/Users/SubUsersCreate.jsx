...
const SubUserCreate = (props) => {
  ...
  const [dataArrayDealer, setDataArrayDealer] = useState([])

  const [checkControlTable, setCheckControlTable] = useState([])

  const [selectValueСountry, setSelectValueСountry] = useState(1)
  ...
  useEffect(() => {
    if (dataOffice.length > 0) {
      const allId = []
      const searchArrayDealer = dataOffice.find(
        (elem) => +elem.id === +selectValueUser
      )
      searchArrayDealer.dealers.map((el) => allId.push(el.id))
      setDataArrayDealer(searchArrayDealer ? searchArrayDealer.dealers : [])
      setCheckControlTable(allId ? allId : [])
    }
  }, [selectValueUser])
  ...
  let params = {
    ...
    //new Params=>
    dealersService: checkControlTable,
    //
  }
                  ...
                  {JSON.parse(window.sessionStorage.getItem('role')).code !==
                    ...
                    <React.Fragment>
                      ...
                      <label>
                        ...
                        <input
                          type="button"
                          value="Список дилеров"
                          onClick={() => setViewDealer(!viewDealer)}
                        />
                      </label>

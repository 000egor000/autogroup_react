...
import { Check, Close, Send } from '@rsuite/icons'
...
const LogistCreate = () => {
  ...
  useEffect(() => {
    ...
    getRequest('/api/v1/user-role', {
      ...
      .then((res) => {
        let filter = res.userRole.filter((el) => el.id === 4)[0][
          'access_rights'
        ]
  ...
  const masterCreateForm = (e) => {
    ...
    e.preventDefault()
    postRequest('/api/v1/logists', params)
      .then((res) => {
        ...
        <div className="top-item " style={{ paddingLeft: state.width }}>
          <div className="btnTransport" style={{ justifyContent: 'left' }}>
            <div className="btnTransportLeft">
              <PagePreviousIcon onClick={() => history(-1)} />
            </div>
          </div>
        </div>
